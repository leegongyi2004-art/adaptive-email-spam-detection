...
A deliberate design objective was that the detector should train and run on ordinary,
inexpensive hardware rather than on specialised accelerators. Because the system used
classical machine-learning models — logistic regression and support-vector machines over
TF-IDF features — rather than deep neural networks, all training, evaluation and inference
were performed on the central processing unit, and no graphics processing unit was required.
This contrasted with deep-learning spam detectors, whose training and inference typically
demand a graphics processor, and it directly supported the project goal of a system that
could be reproduced on a consumer laptop and deployed on modest on-premises or edge mail
infrastructure. The per-message timing measured on this hardware is reported in Chapter 6,
where the median inference latency was approximately 16 milliseconds per email.

The development and evaluation machine was a consumer laptop; the exact processor model and
installed memory are noted in Table 5.1. The complete workflow was also validated on a Linux
environment to confirm that it did not depend on a single platform. Table 5.1 summarises the
...
| Memory (RAM) | [FILL IN: installed RAM, e.g. 8 GB / 16 GB] |
| Graphics | Integrated graphics; no discrete GPU (not required) |
