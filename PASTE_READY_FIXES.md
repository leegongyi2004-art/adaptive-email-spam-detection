...
Your 5.4 has only three figures. The current version has five numbered subsections, one per
figure, each with an explicit "Figure 5.x shows..." elaboration, which is what the marking
guidance on diagrams asks for. Section 5.3 is included so the step numbering matches. Only do
